...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        # --------------------------------------------------------------
        h1 = rg.Point(intersection_center.x+((1/2)*width),
                      (intersection_center.y-(letter_thickness)))
        h2 = rg.Point(intersection_center.x-((1/2)*width),
                      intersection_center.y)
        self.h_rect = rg.Rectangle(h1,h2)
        v1 = rg.Point(intersection_center.x+((1/2)*letter_thickness),
                      intersection_center.y)
        v2 = rg.Point(intersection_center.x-((1/2)*letter_thickness),
    ...
    def clone(self):
        ...
        height = self.v_rect.corner_2.y
        letter_thickness = self.h_rect.corner_1.y - self.h_rect.corner_2.y
        intersection_center = rg.Point(self.h_rect.corner_1.x + (0.5*width),
                                       self.h_rect.corner_2.y)
        tClone = CapitalT(intersection_center, width, height, letter_thickness)
        return tClone
